...
def compose_all(items, config, date_str):
    ...
    prompt = f"""你是软件工程本科生的晨间消息整理助手。北京时间 {date_str}。

你必须严格按下面的格式输出，不得增加任何额外说明文字，不得修改分隔符。
...
然后是一行趋势概括（以 **趋势:** 开头），接着是新闻条目，每条以 · 开头，启示以 → 开头，另起一行。

当你完成 AI 产业板块后，必须输出独占一行的：
===
...
你必须严格按以下格式输出周五特辑，不得添加任何额外说明、提示或修改分隔符。
...
然后输出精选论文。每篇论文必须严格按以下三行格式输出，缺一不可：
- 原文标题
- 原文摘要（翻译成中文）
...
当你完成本周论文板块后，必须独占一行输出：
===

下一行必须是板块标题：
🏫 保研&校招动态

然后逐条列出保研、夏令营、推免、校招相关信息，每条用一句话说明。
...
禁止输出除以上结构外的任何解释、评语或格式说明。总字数控制在2000字以内。"""
    resp = client.chat.completions.create(
        model=config["openai_model"],
