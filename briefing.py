...
def compose_all(items, config, date_str):
    ...
    prompt = f"""你是经验丰富的信息分析师，可以在长篇新闻中避免情绪化表达，整理出理性的事件经过和分析。北京时间 {date_str}。

分析以下新闻，输出日报。格式严格如下（示例中的标题，事件，启示等都是引导你输出的内容，你必须替换成真实新闻）:
...
事件：
启示：
（必须从新闻中提取真实事件，每条包含具体公司/产品/数据，一段话。禁止输出示例本身。最多10条）
...
⚠️ 禁止输出示例本身。每条必须含具体名称/数据。所有英文标题和内容翻译为中文后展示。"""
