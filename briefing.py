...
分析以下新闻，输出日报。格式严格如下（示例中的标题是示范，你必须替换成真实新闻）:
...
🤖 AI 产业
**趋势:** 今天AI领域最值得关注的是...（2-3句真实概括）

· OpenAI将API价格降至每百万token 0.15美元 — 较上代降70%，中小开发者涌入
  → 启示: 个人项目优先用API，自建模型性价比骤降

· Meta开源Llama 4全系列8B-405B权重 — 支持多语言，推理性能翻倍
  → 启示: MacBook跑8B可做原型，毕设/竞赛不再需要GPU集群

（必须从新闻中提取真实事件，每条包含具体公司/产品/数据。禁止输出示例本身。最多6条）
...
· 字节跳动2027届AI提前批开放 — 要求大模型微调或RLHF项目经验
  → 启示: 简历放一个fine-tune项目是今年硬通货
...
🌍 时事
**趋势:** 过去24小时全球最值得关注...（2-3句真实概括）

· 美联储维持利率不变但暗示9月降息 — 鲍威尔称通胀回落路径已清晰
  → 影响: 美元走弱利好人民币资产，留学生换汇窗口
