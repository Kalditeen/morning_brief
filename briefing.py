...
下面会提供一批新闻，每条开头都有分类标记：
[ai]    → 归入「🤖 AI 产业」板块
[job]   → 归入「💼 就业水温」板块
[world] → 归入「🌍 时事」板块

你必须严格按照这三个板块的顺序输出日报，即使某个板块新闻较少，也**必须输出该板块的标题和趋势概括**，不得省略任何板块。
禁止输出任何额外的说明、括号注释或修改板块标题。

输出格式如下（直接照此结构，不要添加任何多余文字）：

📰 晨间简报 | {date_str}

🤖 AI 产业
**趋势:** （基于[ai]类新闻，用2-3句话概括今天AI领域动向）
...
🌍 时事
**趋势:** （基于[world]类新闻，用2-3句话概括24h全球变化）
...
def compose_friday_special(items, config, date_str):
    ...
    prompt = f"""你是软件工程本科生的学术保研助手。北京时间{date_str}。

下面提供两类信息：
- 本周论文（来自 ArXiv / Reddit）
- 保研 & 校招信息（来自保研、夏令营、校招相关资讯）

你必须严格按以下结构输出周五特辑，即使某一板块内容较少，也**必须输出该板块的标题**，不得省略或添加额外说明。
...
📊 论文{len(academic)}篇 + 保研/就业{len(baoyan)}条
"""
    resp = client.chat.completions.create(
        model=config["openai_model"],
...
def compose_wechat_summary(items, config, date_str):
    ...
    prompt = f"""你是晨间简报摘要助手。{date_str}。阅读以下新闻，按照软件工程就业相关度选出至多3条最重要/最热点的事件，用一句话概括每条（· 开头）。突出事件本身和为什么值得关注。总字数150字内，不要标题和解释。
